...
def data_yolo(folder, dest, size):
    ...
    for d in sorted(dirs):  # ciclo su video
        ...
        for f in sorted(dirs_frame):
            ...
            if 'png' == f.split('.')[1] and f.split('.')[
                0] != 'video0001':  # non voglio il 1° frame perche non ho l'event
                print('Scale_image: ', path_image)
                #image_resize = resize_path + f + '.png'
                #scale_image(path_image, image_resize, size)
                print('Calculate bb')
                bb = bounding_box(path_image)
                print('Finish bb: ', bb)
                if bb.size != 0:  # non salvo se non c'è bb
                    name_file = d + f.split('.')[0]  # chiamo i file video#.txt
                    create_ann(bb, name_file, dest, size)
                else:
...
# uso video scalaati ! non serve
def scale_image(frame, new_name, size):
...
def create_ann(coord, row, dest, size):
    center = [((coord[0][0] + coord[1][0]) / 2) / size, ((coord[0][1] + coord[2][1]) / 2) / size]
    w = (coord[0][0] - coord[1][0]) / size  # (xmax - xmin)
    h = (coord[0][1] - coord[2][1]) / size  # (ymax - ymin)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--size", dest="size", default=None, help="Size of image")
    args = parser.parse_args()
    print('Video: ', args.video)
    data_yolo(args.video, args.dest, args.size)
